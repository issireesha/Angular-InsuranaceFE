...
@Component({
  selector: 'app-buymedicalform',
  templateUrl: './buymedicalform.component.html',
  styleUrls: ['./buymedicalform.component.css']
})
export class BuymedicalformComponent implements OnInit {
...
constructor(private actRoute:ActivatedRoute,private service:MedicalplanService,
...
buynewmedicalplan(){
  ...
  if(age>0 && age<=15){
    ...
    Swal.fire("Medical Insurance is not available for people aged above 70");
    return;
  }
        ...
        if(result.isConfirmed){
          this.pending.addPendingInsurance(this.primary_subscriber).subscribe(
            (data)=>{
              ...
              Swal.fire("Thank You !","Purchased Insurance Successfully.","success");
            },
            (error)=>{
